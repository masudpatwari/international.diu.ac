...
class MailReader
{
  ...
  {
    $imap_host  = env('IMAP_HOST'); // IMAP host address
    $imap_flags =  env('IMAP_FLAGS'); // IMAP Flags
    $imap_user  =  env('IMAP_USER'); // IMAP username
    $imap_pass  =  env('IMAP_PASS'); // IMAP password
    $file_store = env('EMAIL_ATTACHMENT_DIR_PATH'); // Folder where file attachments are saved
    ...
    if ($inbox) {
      ...
      if ($emails) {
        ...
        foreach($emails AS $n) {
          ...
          $uniqid = uniqid() . '-' . date("U");
          $emailMessage = new EmailObject($uniqid,$source,$file_store);
          $emailContents[] = $emailMessage->readEmail();
          imap_delete($inbox, $n);
        }

        // uncomment bellow line after test *** imp ***
        //imap_expunge($inbox);
    ...
    function _parseHeaders($input)
    {
        ...
        if ($input !== '') {
            ...
            foreach ($headers as $value) {
                ...
                /*if(is_array($hdr_value) && $hdr_value[0] == ' ') {
...
class EmailObject {
  ...
  function __construct($uniqid,$source,$file_store) {
  ...
  function readEmail(){
    ...
    if ( isset($this->decoded->parts) && is_array($this->decoded->parts) && count($this->decoded->parts)>0) {
  ...
  private function decodePart($body_part){
    ...
    #echo EOL .EOL .  $body_part->ctype_primary;
  ...
  // Save file
  private function saveFile($filename,$emailContents) {

    // Check if uniqid folder exists
    if (!file_exists($this->file_store."/".$this->uniqid))
      mkdir($this->file_store."/".$this->uniqid);
    // Save file
    file_put_contents($this->file_store."/".$this->uniqid."/".$filename, $emailContents);
  ...
  private function getOutput() {
    ...
    if (isset($this->bodyText)) {
      ...
      $body_text = mb_convert_encoding(trim($body_text),'UTF-8','UTF-8');
    } else {
      $body_text = "";
    }
    ...
    if (isset($this->bodyHtml)) {
      ...
      $body_html = mb_convert_encoding(trim($body_html),'UTF-8','UTF-8');
    } else {
      $body_html = "";
    }
    ...
    if (isset($this->email))
      $email = mb_convert_encoding($this->email,'UTF-8','UTF-8');
    else
      $email = "";
    ...
    if (sizeof($this->saved_files) > 0) {
      foreach($this->saved_files as $filename){
        $filenames_array[] = mb_convert_encoding($filename,'UTF-8','UTF-8');
      }
    }
    ...
    return [
      ...
      'from_email'=> $email,
      'from_name'=> $name,
      'body'=>  isset($this->bodyText)? $body_text: $body_html,
